...
async def amain():
	...
	# URL-encode the username, password, and IP address
	encoded_username = quote(args.username)
	encoded_password = quote(args.password)
	encoded_ip_address = quote(args.ip_address)
	
	# Generate the RDP connection URL string
	rdp_url = f"rdp+ntlm-password://{encoded_username}:{encoded_password}@{encoded_ip_address}"
